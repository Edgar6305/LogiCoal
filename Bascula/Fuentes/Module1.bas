...
Global sPathMasivo As String
Global sPathComprobantes As String
...
'    Conn.Properties("password") = "A$123bcd2022"
    ...
    If Now() < #10/12/2022 5:00:00 PM# Then
...
Public Function CargaCombo(Combo1 As Object, Tipo As Integer)
Dim xR As New ADODB.Recordset
Dim xSql As String

xSql = "Select Descripcion From DescriptivosDetalle Where IDTipo=" & Tipo & "Order By Descripcion"
Set xR = Conn.Execute(xSql)
Combo1.Clear

If Not xR.EOF Then
    Combo1 = xR!Descripcion
    Do While Not xR.EOF
        Combo1.AddItem xR!Descripcion
        xR.MoveNext
    Loop
    CargaCombo = 0
Else
...
Public Function Completa(xBuff, xLen As Integer, xPad As String) As String
Dim i As Integer, xT As Integer
On Error Resume Next
xT = xLen - Len(xBuff)
For i = 1 To xT
     xBuff = xBuff & xPad
