...
Else
    ...
    Case 5
        ...
        If Conn.Execute("SELECT TransaccionOrigen FROM Bascula WHERE IdTiquete=" & SnumeroTiquete).Fields(0) = "DS" Then ' <== VENTAS
            ...
            xSql = xSql + "                  Ventas.OrdenCompraCliente , Acopios.Descripcion AS DesAcopio, Acopios.Ubicacion, Ventas.CantidadPedida, Ventas.CantidadDespachada , Usuarios_T.Descripcion AS DesUsuario_T, Usuarios_T.Cargo, Conductores.Nombre"
            xSql = xSql + " FROM    Bascula INNER JOIN"
        ...
        Else
            ...
            xSql = xSql + "                    Acopios.Descripcion AS DesAcopio, Acopios.Ubicacion, Traslados.Fecha AS FechaOrdenTraslado, Bascula.Usuario, Usuarios_T.Descripcion AS DesUsuario, Usuarios_T.Cargo, Conductores.Nombre"
...
Select Case TiempoTran
         ...
         Case 2               '==>  Recepcion Peso Vacio
                xSql = " EXEC PA_PesoFinal " & TiempoTran & ", " & SnumeroTiquete & ", " & MemVar_8.text & ",'" & MemVar_51 & "','" & MemVar_4 & "'"
         ...
         Case 5        '==>  Despachos Peso Lleno
                xSql = " EXEC PA_PesoFinal " & TiempoTran & ", " & SnumeroTiquete & ", " & MemVar_7.text & ",'" & MemVar_51 & "','" & MemVar_4 & "'"
                Res = Conn.Execute(xSql).Fields(0)
                If Res <> "OK" Then
                    MsgBox "Error al Grabar el Registro, Verifique" & vbCrLf & Res, vbInformation, "Error de Grabación"
                End If
