...
Public Property Get Image(Optional ByVal Width As Long, Optional ByVal Height As Long) As IPicture
    ...
    If m_ChartOrientation = CO_Vertical Then
        DrawVertical TempDC, Width, Height
    Else
        DrawHorizontal TempDC, Width, Height
    End If
...
Private Sub UserControl_Paint()
    If m_ChartOrientation = CO_Vertical Then
        DrawVertical UserControl.HDC, UserControl.ScaleWidth, UserControl.ScaleHeight
    Else
        DrawHorizontal UserControl.HDC, UserControl.ScaleWidth, UserControl.ScaleHeight
    End If
End Sub
...
End Function
'*1
Private Sub DrawVertical(HDC As Long, ScaleWidth As Long, ScaleHeight As Long)
    ...
    If m_AxisXVisible Then
        For i = 1 To cAxisItem.Count
            TextWidth = UserControl.TextWidth(IIf(IsNull(cAxisItem(i)), 1, cAxisItem(i))) * 1.3
            TextHeight = UserControl.TextHeight(IIf(IsNull(cAxisItem(i)), 1, cAxisItem(i))) * 1.3
...
'    End If
    ...
    For i = forLines / (iStep * NumDecim) To toLines / (iStep * NumDecim)
        If m_HorizontalLines Then
            GdipDrawLine hGraphics, hPen, MarginLeft, YY, ScaleWidth - MarginRight - mPenWidth, YY
        End If
        ...
        If (m_ChartStyle = CS_StackedBars) Or (m_ChartStyle = CS_StackedBarsPercent) Then
            ...
            Else
                ...
                For j = 0 To m_Serie(i).Values.Count - 1
                    ...
                    With m_Serie(i).Rects(j)
                        .Left = MarginLeft + PtDistance * j + BarWidth / 2
                        If Value >= 0 Then 'Edgardo Hernandez
                            If Max > 0 Then
                                .Height = (Value * (Max * (ZeroPoint - TopHeader) / toLines) / Max)
                            Else
                                .Height = 0 '(Value * (Max * (ZeroPoint - TopHeader) / toLines) / Max)
                            End If
                            .Top = LastPositive(j) - .Height
                            LastPositive(j) = .Top
                        Else
    ...
    GdipDrawLine hGraphics, hPen, MarginLeft, ZeroPoint, ScaleWidth - MarginRight - mPenWidth, ZeroPoint
    GdipDeletePen hPen
...
End Sub
'*3
Private Sub DrawHorizontal(HDC As Long, ScaleWidth As Long, ScaleHeight As Long)
    Dim hGraphics As Long, hPath As Long
    Dim hBrush As Long, hPen As Long
    Dim mRect As RectL
    Dim Min As Single, Max As Single
